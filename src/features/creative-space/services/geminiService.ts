...
export const generateAudio = async (
    ...
    return logAPICall(
        'generateAudio',
        async () => {
            // 不再强制要求 Gemini，支持 OpenRouter 等其他提供商
            const currentProvider = llmProviderManager.getCurrentProviderType();
            
            // 如果是 Gemini 提供商，使用原生 Gemini API
            if (currentProvider === 'gemini') {
                const ai = getClient();
                ...
                return combineBase64Chunks([audioData], sampleRate);
            }
            
            // 对于 OpenRouter 等其他提供商，使用 LLM 文本生成
            // Lyria 等模型通过 OpenRouter 的 chat completions API 返回音频 URL 或 base64
            const llmProvider = llmProviderManager.getCurrentProvider();
            const audioResponse = await llmProvider.generateContent(
                prompt,
                model,
                options?.persona ? { systemInstruction: options.persona.desc } : undefined
            );
            
            // 如果返回的是 URL 或 base64 音频数据，直接返回
            if (audioResponse.startsWith('http://') || audioResponse.startsWith('https://') || audioResponse.startsWith('data:')) {
                return audioResponse;
            }
            
            // 否则将文本回答作为音频提示词返回（由前端处理）
            throw new Error('OpenRouter 音频生成暂不支持，请使用 Gemini API 进行音频生成');
        },
        {
            model,
            prompt: prompt.substring(0, 200) + (prompt.length > 200 ? '...' : '')
        },
        { platform: llmProviderManager.getCurrentProvider().getName(), logType: 'submission' }
    );
};
