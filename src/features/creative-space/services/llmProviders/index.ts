...
class LLMProviderManager {
  ...
  getCurrentProviderType(): LLMProviderType {
    ...
    // 如果配置了 OpenRouter API，默认使用 openrouter
    const openrouterKey = localStorage.getItem('OPENROUTER_API_KEY');
    const oldOpenrouterKey = localStorage.getItem('CUSTOM_API_KEY');
    const openrouterUrl = localStorage.getItem('CUSTOM_API_URL');
    if (openrouterKey || (oldOpenrouterKey && openrouterUrl?.includes('openrouter'))) {
      return 'openrouter';
    }
    return savedProvider || 'openrouter'; // 默认使用 OpenRouter
  }
  ...
  isCurrentProviderConfigured(): boolean {
    ...
    if (providerType === 'openrouter') {
      const apiKey = localStorage.getItem('OPENROUTER_API_KEY') || localStorage.getItem('CUSTOM_API_KEY');
