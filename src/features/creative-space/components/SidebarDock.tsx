...
import {
    ...
    Edit, Trash2, MoreHorizontal, Copy, Download, Box, ScanFace, Brush, Type, Workflow as WorkflowIcon,
...
export const SidebarDock: React.FC<SidebarDockProps> = ({
    ...
    const commitWorkflowRename = (id: string, value: string) => {
        const title = value.trim();
        if (!title) {
            setEditingWorkflowId(null);
            return;
        }
        onRenameWorkflow(id, title);
        setEditingWorkflowId(null);
    };
        ...
        if (activePanel === 'workflow') {
            return (
                <>
                    ...
                    <div className="flex-1 overflow-y-auto p-2 custom-scrollbar space-y-1 relative">
                        {workflows.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-10 text-slate-500 opacity-60 select-none">
                            ...
                            workflows.map(wf => (
                                <div 
                                    key={wf.id} 
                                    className={`
                                        relative px-2.5 py-2 rounded-lg border bg-black/10 group transition-all duration-300 cursor-grab active:cursor-grabbing hover:bg-white/5
                                ...
                                >
                                    <button
                                        ...
                                        className="absolute top-2.5 right-2 z-20 w-5 h-5 rounded bg-black/60 border border-white/10 text-white/70 hover:text-white hover:bg-white/15 hover:border-white/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all cursor-pointer"
                                        title="更多操作"
                                        aria-label="更多操作"
                                    >
                                    ...
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            {editingWorkflowId === wf.id ? (
                                                <input 
                                                    className="bg-black/50 border border-cyan-500/50 rounded px-1.5 py-0.5 text-xs text-white w-full outline-none"
                                                    defaultValue={wf.title}
                                                    autoFocus
                                                    onClick={(e) => e.stopPropagation()}
                                                    onDoubleClick={(e) => e.stopPropagation()}
                                                    onBlur={(e) => commitWorkflowRename(wf.id, e.target.value)}
                                                    onKeyDown={(e) => {
                                                        if (e.key === 'Enter') commitWorkflowRename(wf.id, e.currentTarget.value);
                                                        if (e.key === 'Escape') setEditingWorkflowId(null);
                                                    }}
                                                />
                                            ) : (
                                                <div className="text-xs font-medium text-slate-300 truncate select-none group-hover:text-white transition-colors" title="双击重命名">{wf.title}</div>
                                            )}
                                            <div className="text-[9px] text-slate-600 font-mono">{wf.nodes.length} 节点</div>
                                        </div>
                                    </div>
                                </div>
                            ))
        ...
        return (
            <>
                ...
                <div className="flex-1 overflow-y-auto p-2 custom-scrollbar space-y-2">
                    {[NodeType.SCRIPT_PLANNER, NodeType.SCRIPT_EPISODE, NodeType.CHARACTER_NODE, NodeType.STORYBOARD_IMAGE, NodeType.STORYBOARD_SPLITTER, NodeType.STORYBOARD_VIDEO_GENERATOR, NodeType.DRAMA_ANALYZER, NodeType.VIDEO_EDITOR].map(t => {
        ...
        <>
            ...
            >
                ...
                {[
                    ...
                    { id: 'workflow', icon: FolderHeart, tooltip: '我的工作流' },
                    ...
                    { id: 'history', icon: History, tooltip: '历史素材' },
                ...
                >
                    ...
                    {contextMenu.type === 'workflow' && (
                        <>
                            ...
                            <button className="w-full text-left px-3 py-2 text-xs text-slate-200 hover:bg-white/10 rounded-md flex items-center gap-2" onClick={() => { const wf = workflows.find(w => w.id === contextMenu.id); if (wf) navigator.clipboard?.writeText(JSON.stringify(wf, null, 2)); setContextMenu(null); }}>
                                <Copy size={12} /> 复制信息
                            </button>
                            <button className="w-full text-left px-3 py-2 text-xs text-slate-200 hover:bg-white/10 rounded-md flex items-center gap-2" onClick={() => { const wf = workflows.find(w => w.id === contextMenu.id); if (wf) { const blob = new Blob([JSON.stringify(wf, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${wf.title || 'workflow'}.json`; a.click(); URL.revokeObjectURL(url); } setContextMenu(null); }}>
                                <Download size={12} /> 导出 JSON
                            </button>
                            <div className="my-1 h-px bg-white/10" />
                            <button className="w-full text-left px-3 py-2 text-xs text-red-400 hover:bg-red-500/20 rounded-md flex items-center gap-2" onClick={() => { const wf = workflows.find(w => w.id === contextMenu.id); if (wf && confirm(`确定删除「${wf.title}」吗？`)) onDeleteWorkflow(contextMenu.id); setContextMenu(null); }}>
                                <Trash2 size={12} /> 删除
                            </button>
                        </>
                    )}
