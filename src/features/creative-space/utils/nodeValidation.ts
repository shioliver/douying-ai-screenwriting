...
export const NODE_DEPENDENCY_RULES: Record<NodeType, {
  ...
  [NodeType.IMAGE_GENERATOR]: {
    allowedInputs: [
      ...
      NodeType.SCRIPT_PLANNER  // 支持从剧本大纲直接输出
    ],
    allowedOutputs: [
  ...
  [NodeType.IMAGE_EDITOR]: {
    allowedInputs: [
      ...
      NodeType.IMAGE_EDITOR,
      NodeType.STYLE_PRESET,  // 接入风格设定
      NodeType.SCRIPT_PLANNER  // 支持从剧本大纲直接输入
    ],
    allowedOutputs: [
      NodeType.IMAGE_GENERATOR,
      NodeType.VIDEO_GENERATOR,
  ...
  [NodeType.SCRIPT_PLANNER]: {
    ...
    allowedOutputs: [
      ...
      NodeType.STORYBOARD_GENERATOR,  // 直接输出到分镜生成
      NodeType.STORYBOARD_IMAGE,  // 直接输出到分镜图设计
      NodeType.IMAGE_GENERATOR,  // 直接输出到图像生成
      NodeType.VIDEO_GENERATOR,  // 直接输出到视频生成
      NodeType.AUDIO_GENERATOR,  // 直接输出到音频生成
      NodeType.SORA_VIDEO_GENERATOR,  // 直接输出到 Sora 视频生成
      NodeType.STORYBOARD_VIDEO_GENERATOR,  // 直接输出到分镜视频生成
      NodeType.STORYBOARD_SPLITTER,  // 输出到分镜拆解
      NodeType.IMAGE_EDITOR  // 输出到图像编辑
    ],
    minInputs: 0,
    maxInputs: 3,
    description: '生成剧本大纲,可直接输出到各种生成节点'
  },
...
export function validateConnection(
  ...
  console.log('🔍 验证连接:', {
      fromNode: { id: fromNode.id, type: fromNode.type, title: fromNode.title },
      toNode: { id: toNode.id, type: toNode.type, title: toNode.title }
  });
  ...
  console.log('📋 fromRules.allowedOutputs:', fromRules.allowedOutputs);
  console.log('📋 toRules.allowedInputs:', toRules.allowedInputs);

  if (!fromRules.allowedOutputs.includes(toNode.type)) {
    console.log('❌ fromRules 检查失败');
    return {
      valid: false,
  ...
  console.log('🔄 重复连接检查:', {
      duplicateConnection,
      existingConnections: existingConnections.map(c => `${c.from} → ${c.to}`)
  });
